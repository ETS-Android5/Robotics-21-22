...
abstract class FourWheelRobot(val hardwareMap: HardwareMap) {
     ...
     * values in a FourWheelBuffer.
     * Used for more abstract motions.
     * The buffer is clamped to 1.0 before using it.
     * See FourWheelBuffer.clampToValue for more information.
     */
    fun move(buffer: FourWheelBuffer) {
        val modifiedBuffer = buffer.clampToValue(1.0)
        for ((value, motor) in modifiedBuffer.values zip wheels) {
            motor.power = value
        }
    }
    // A convenience method for calling buffer functions
    // in the context of a specific robot.
